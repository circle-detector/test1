...
# السماح بفتح الصور الكبيرة جدًا
...
if uploaded_file is not None:
    try:
        image = Image.open(uploaded_file).convert("RGB")  # التأكد من 3 قنوات
        st.image(image, caption="📷 الصورة الأصلية", use_container_width=True)
        count, result_image = detect_irrigation_circles(image)
        st.image(result_image, caption=f"🟢 عدد دوائر الري المحوري المكتشفة: {count}", use_container_width=True)
